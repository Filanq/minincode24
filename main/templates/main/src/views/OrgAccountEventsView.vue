<template>
    ...
    <ModalCreateComponentEvent v-if="showCreateModal" @close="()=>{changeCondCreateModal(); load_events();}" :changeCondCreateModal="changeCondCreateModal" />
    <div class="container-with-header">
        <HelloAccountComponent :name="user_name" />
        <div class="inner-container">
            <div class="title-add-container">
                <h3 class="h5">Мероприятия организации</h3>
            ...
            <div class="news__main_wrap gap-25">
                <div class="news__main_block" v-for="event in events">
                    ...
                    <div class="news__info">
                        ...
                        <div @click="remove(event.id)" class="news-info__item delete-event"><img src="@/assets/images/icon/close.svg" alt=""></div>
                    </div>
                </div>
            </div>
        </div>
...
<script setup lang="ts">
    ...
    import ModalCreateComponentEvent from "@/components/ModalCreateComponentEvent.vue";
    import {ref} from 'vue';
    ...
    const events = ref([]);
    const load_events = () => {
        axios.get(window.origin + '/api/events').then(async res => {
            events.value = res.data.filter((i)=>{return i.organization === Number(user.id)});
        });
    };
    user.login().then(()=>{
    ...
    const remove = (id) => {
        events.value.splice(events.value.indexOf(events.value.filter((item)=>{return item.id == id})[0]), 1);
        axios.delete(window.origin + '/api/events/' + id + '/');
    };
</script>
