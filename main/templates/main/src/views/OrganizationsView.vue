    ...
    <main>
        <section class="section organization__section">
            <div class="container organizations__container gap-50">
              ...
              <input v-model="query" type="text" class="organization__input input js-s width-100" placeholder="Поиск">
                <div class="organizations__inner gap-25 grid width-100">
                    <div class="organizations__wrap grid grid-column gap-10 ac-s width-100">
                        <div @click="selectActive(org.id)" :class="{'organization_active': active_org === org.id}" class="organization__item grid grid-column ji-s ac-s" v-for="org in orgs">
                            <h3 class="h5 w-500 organization-item__title">{{ org.name }}</h3>
                            <span class="h7 w-300 organization-item__address">{{ org.address }}</span>
                        </div>
                    </div>
      ...
      <section class="section news__section" v-show="news.length">
      ...
      <section class="section events__section" v-show="events.length">
        <div class="container events__container gap-50">
          ...
          <div class="events__main_wrap gap-25">
            <router-link to="" target="_blank" class="events__main_block" v-for="cur_events in events">
...
let active_org = ref(0);

let query = ref('');

watch(query, () => {
    orgs.value = users.filter((i)=>{return (i.name + ' ' + i.address).includes(query.value) && i.type === 'organization';});
});
...
axios.get(window.origin + '/api/users').then(res => {
    users = res.data;
    orgs.value = users.filter((i)=>{return i.type === 'organization'});
});
...
const selectActive = (id) => {
    ...
    axios.get(window.origin + '/api/news').then(res => {
      news.value = res.data.filter((i)=>{return i.organization_id === active_org.value});
    });

    axios.get(window.origin + '/api/events').then(res => {
      events.value = res.data.filter((i)=>{return i.organization_id === active_org.value});
    });
};
    ...
    watch(orgs, ()=> {
        function initializeYandexMap() {
            ymaps.ready(async () => {
                ...
                for(let org of orgs.value){
                    let geocoder = ymaps.geocode(org.address);
                geocoder.then(
                    function (res) {
                        ...
                        if (firstGeoObject) {
                            ...
                            var placemark = new ymaps.Placemark(coordinates, {
                                ...
                                iconImageHref: '/assets/img/organization.svg',
